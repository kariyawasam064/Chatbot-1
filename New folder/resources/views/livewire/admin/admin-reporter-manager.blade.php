...
>
    ...
    <tr>
        ...
        <td>
                <button type="button" class="btn btn-link" data-bs-toggle="modal" data-bs-target="#updateReporterModal" onclick="editReporter('{{ $reporter->emp_id }}'); setLoadingIndicator(true)">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const confirmDiscard = (event) => {
        if (isFormStateChanged()) {
            ...
            if (!confirmation) {
                event.preventDefault(); // Prevent the default action, which is closing the modal
                return false; // Ensure the modal stays open
            } else {
    ...
    cancelButton.addEventListener('click', (event) => {
        confirmDiscard(event); // Call confirmDiscard to confirm if the user wants to discard changes
        if (isFormStateChanged()) {
            // Cancel button behavior: prevent modal close
            event.stopImmediatePropagation(); // Stop the modal from closing when cancel is clicked
        }
    });
    ...
    closeButton.addEventListener('click', (event) => {
        confirmDiscard(event); // Call confirmDiscard to confirm if the user wants to discard changes
        if (isFormStateChanged()) {
            // Close button behavior: prevent modal close
            event.stopImmediatePropagation(); // Stop the modal from closing when close is clicked
        }
    });
    ...
    // Handling the search functionality and updating the table dynamically
    document.getElementById('searchBtn').addEventListener('click', function () {
        ...
        fetch(`{{ route('reporters.filter') }}?empID=${empID}&group=${group}`)
            ...
            .then(data => {
                ...
                const noReportersMessage = `
                    <tr>
                        <td colspan="5" class="text-center">No reporters found</td>
                    </tr>`;
                if (data.length === 0) {
                    tableBody.innerHTML = noReportersMessage;
                } else {
